...
def gaze(frame, points):
    ...
    if points is None or points.landmark is None:
        return

    # Check if the necessary landmarks are available
    required_landmarks = [4, 152, 263, 33, 287, 57]  # Example indices of required landmarks
    if any(index >= len(points.landmark) for index in required_landmarks):
        return

    # Access the required landmarks
    ...
    additional_landmarks = [468, 473]  # Example indices of additional landmarks
    if any(index >= len(points.landmark) for index in additional_landmarks):
        return

    # Access the additional landmarks
    ...
    if transformation is not None:  # if estimateAffine3D succeeded
        ...
        # Smooth the gaze position using exponential moving average (EMA)
        if prev_gaze is not None:
            gaze = (alpha * gaze) + ((1 - alpha) * prev_gaze)
